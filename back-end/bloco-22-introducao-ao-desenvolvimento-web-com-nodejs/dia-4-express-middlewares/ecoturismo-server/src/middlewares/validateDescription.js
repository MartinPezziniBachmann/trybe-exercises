const validateDescription = (req, res, next) => {
  ...
  if (!description.createdAt) return res.status(400).json({
    message: 'O campo createdAt é obrigatório'
  });
  if (!description.rating) return res.status(400).json({
    message: 'O campo rating é obrigatório'
  });
  if (!description.difficulty) return res.status(400).json({
